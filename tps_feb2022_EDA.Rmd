...
We propose a rapid overview of some statistics. Note that we scaled the data by a factor x 1000 to reduce numbers of zero after the commas. 
...
The aim of this analysis is to identify the top twenty (20) most related features to each bacteria species. For this we use the correlation matrix, the correlations are directly related to the coefficients of a basic linear regression.
...
for(i in seq_along(target_encode)) {
  target_encode[[i]] <- 
...
p <- vector("list", length(bacteria_species))

for(i in seq_along(p)) {
  p[[i]] <- featCorToTarget_plot(x_corr, bacteria_species[[i]])
}
...
> * third, this table summarize the results :

```{r}
top_cor <- function(x, feat) {
  x <- x
  top <- max(x)
  feat <- feat[x == top]
  list(top, feat)
...
tab <- lapply(x_corr, top_cor, feature)

top_cor <- rep(NA, length(tab))
feat <- rep(NA, length(tab))

for(i in seq_along(tab)) {
    top_cor[[i]] <- pluck(tab, i, 1)
    feat[[i]] <- pluck(tab, i, 2)
}

tidy <- tibble(bacteria_target_species = names(tab),
               top_correlation = top_cor,
               feature_correlated_to = feat) %>% 
...
tidy %>% 
  gt() %>% 
  cols_align("center", contains("scale")) %>% 
  fmt_number(columns = "top_correlation", decimals = 3) %>% 
  tab_header(title = "Top couple of bacteria and feature correlation") %>% 
  gt_theme_538()
...
p <- vector("list", length(features))

sample <- bacteria_train %>% sample_n(10000)

for(i in seq_along(p)) {
  
  p[[i]] <- density_plot(sample, features[[i]])
